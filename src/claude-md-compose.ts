...
export function composeGroupClaudeMd(group: AgentGroup): void {
  ...
  // Project .claude/rules/ — compaction anchors (active-phase, active-knowledge,
  // working-knowledge). Inline since no container mount for project root .claude/.
  const rulesDir = path.join(process.cwd(), '.claude', 'rules');
  if (fs.existsSync(rulesDir)) {
    for (const entry of fs.readdirSync(rulesDir)) {
      if (!entry.endsWith('.md')) continue;
      const rulesPath = path.join(rulesDir, entry);
      try {
        const stat = fs.statSync(rulesPath);
        if (!stat.isFile()) continue;
        desired.set(`rules-${entry}`, {
          type: 'inline',
