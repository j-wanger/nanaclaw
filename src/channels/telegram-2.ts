/**
 * Second Telegram bot adapter — same mechanics as telegram.ts but reads
 * TELEGRAM_BOT_TOKEN_2 and registers as 'telegram2'. Allows a separate
 * bot identity wired to a different agent group.
 */
...
async function withRetry<T>(fn: () => Promise<T>, label: string, maxAttempts = 5): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      log.warn('Telegram2 setup failed, retrying', { label, attempt, delayMs: delay, err });
...
async function fetchBotUsername(token: string): Promise<string | null> {
  try {
    ...
    log.warn('Telegram2 getMe failed', { err });
    return null;
  }
}
...
async function sendPairingConfirmation(token: string, platformId: string): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      log.warn('Telegram2 pairing confirmation non-OK', { status: res.status });
    }
  } catch (err) {
    log.warn('Telegram2 pairing confirmation failed', { err });
  }
}
...
function createPairingInterceptor(
  ...
  return async (platformId, threadId, message) => {
    try {
      ...
      log.info('Telegram2 pairing accepted — chat registered', {
        platformId,
      ...
      log.error('Telegram2 pairing interceptor error', { err });
      hostOnInbound(platformId, threadId, message);
    }
  };
}
...
registerChannelAdapter(CHANNEL_TYPE, {
  factory: () => {
    const env = readEnvFile(['TELEGRAM_BOT_TOKEN_2']);
    if (!env.TELEGRAM_BOT_TOKEN_2) return null;
    const token = env.TELEGRAM_BOT_TOKEN_2;
